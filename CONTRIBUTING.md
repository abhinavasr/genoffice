...
| `GENOFFICE_CLOUD_SLIDE`                                  | Route slide generation through Codex instead of the local renderer     |
| `CODEX_API_KEY`, `CODEX_CLI_PATH`, `CODEX_EXEC_ARGS`      | Codex CLI credentials / binary location / `codex exec` flag overrides  |
| `AI_SEARCH_DISABLE_CODEX`, `SERPER_API_KEY`               | Disable the Codex search backend / supply a Serper key instead         |
