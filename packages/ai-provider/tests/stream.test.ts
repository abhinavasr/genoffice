...
describe('streamForProvider: ollama', () => {
  it('hits the local OpenAI-compatible endpoint by default', async () => {
    const fetchMock = vi.fn().mockResolvedValue(okResponse(sseStream(['data: [DONE]'])))
    vi.stubGlobal('fetch', fetchMock)
    const { cb } = collector()
    await streamForProvider(
      'ollama',
      { apiKey: 'ollama', model: 'gemma4' },
      'sys',
      [],
      [],
      100,
  ...
  it('honors a custom base URL (e.g. a remote Ollama server)', async () => {
    const fetchMock = vi.fn().mockResolvedValue(okResponse(sseStream(['data: [DONE]'])))
    vi.stubGlobal('fetch', fetchMock)
    const { cb } = collector()
    await streamForProvider(
      'ollama',
      { apiKey: 'ollama', model: 'gemma4', baseUrl: 'http://192.168.1.10:11434/v1' },
      'sys',
      [],
      [],
      100,
...
describe('streamForProvider: 200 + non-stream JSON instead of SSE', () => {
  const creditsNotice =
    'Your account credits have been exhausted. Please top up to purchase more credits.'
  ...
  it('gemini route: an insufficient-credits notice counts as a credits notice', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
          candidates: [
            {
              content: {
                parts: [{ text: 'Out of quota: insufficient credit balance, please top up.' }],
              },
            },
          ],
