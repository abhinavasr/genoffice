...
describe('chatForProvider', () => {
  ...
  it('ollama: hits the local OpenAI-compatible endpoint by default', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ choices: [{ message: { content: 'ok' } }] }))
    vi.stubGlobal('fetch', fetchMock)
    await chatForProvider('ollama', { apiKey: 'ollama', model: 'gemma4' }, 'sys', 'hi')
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it('ollama: honors a custom base URL (e.g. a remote Ollama server)', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ choices: [{ message: { content: 'ok' } }] }))
    vi.stubGlobal('fetch', fetchMock)
